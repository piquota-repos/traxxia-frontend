export const categorizeQuestionsWithGroq = async (questions) => {
  ...
  const prompt = `You are an expert business consultant. Your task is to find EXACTLY 7 questions for Phase 1 that cover these SPECIFIC categories:

**REQUIRED CATEGORIES FOR PHASE 1 (exactly 7 questions total):**
...
- EXACTLY 7 questions must go in Phase 1 (one per category above)
- All remaining questions go in Phase 2
...
{
  "phase1": [exactly 7 question numbers],
  "phase2": [all remaining question numbers]
}

CRITICAL: Phase 1 must have exactly 7 questions, one for each required category.`;
  ...
  try {
    ...
    // Ensure exactly 7 questions in Phase 1
    if (categorization.phase1.length !== 7) {
      if (categorization.phase1.length > 7) {
        const extra = categorization.phase1.slice(7);
        categorization.phase1 = categorization.phase1.slice(0, 7);
        categorization.phase2 = [...categorization.phase2, ...extra];
      }
    }
