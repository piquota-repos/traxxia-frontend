...
import { Card, Button, Modal, Form, Container, Row, Col, Accordion, Alert, Navbar, Nav, Dropdown, Badge } from 'react-bootstrap';
import { Star, LogOut, Eye, BarChart, CheckCircle, Home, FileText, PieChart, Settings, User, Bell, Search, Zap, Briefcase, Activity } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const allQuestionsAnswered = () => {
    return Object.keys(answers).every(questionId => isQuestionCompleted(questionId));
  };
  ...
  const getAccordionStyles = (questionId) => {
    return {
      backgroundColor: isQuestionCompleted(questionId) ? 'var(--completed-bg)' : 'white',
      borderLeft: isQuestionCompleted(questionId) ? '4px solid var(--accent-color)' : 'none',
            ...
            <Card key={question.id} className="mb-3 preview-card">
              <Card.Body>
                ...
                {question.type === 'options' && answers[question.id]?.selectedOption && (
                  <p className="text-primary">Selected: {answers[question.id].selectedOption}</p>
                )}
                
                {answers[question.id]?.description && (
                  <p className="text-muted">{answers[question.id].description}</p>
                )}
              </Card.Body>
            </Card>
          ))}
  ...
  const renderPlaceholderContent = (section) => {
    const sectionInfo = {
      reports: {
        ...
        description: "View and download all your survey reports in one place. Generate custom reports and share with your team."
      },
      analytics: {
        icon: <Activity size={48} className="mb-3 text-primary" />,
        description: "Deep dive into your survey data with interactive charts, trends analysis, and AI-powered insights."
      },
      settings: {
        icon: <Settings size={48} className="mb-3 text-primary" />,
        description: "Customize your dashboard, manage notifications, and update your account preferences."
      }
    };
    ...
    return (
      <div className="text-center py-5 placeholder-content">
        <div className="glass-card p-5">
          ...
          <h4 className="mb-3">Welcome to {section.charAt(0).toUpperCase() + section.slice(1)}</h4>
        ...
        <div className="glass-card p-4">
          ...
          {categories.map((category) => (
            <div key={category.id} className="category-section">
              ...
              <Accordion className="mb-4 modern-accordion">
                {category.questions.map((question) => (
                  <Accordion.Item
                    key={question.id}
                    eventKey={question.id}
                    className={isQuestionCompleted(question.id) ? "question-completed" : ""}
                  >
                    <Accordion.Header>
                    ...
                    <Accordion.Body>
                      {/* <div className="star-rating-container">
                        <StarRating
                          currentRating={answers[question.id]?.rating || 0}
                          onRatingChange={(rating) => handleRatingClick(question.id, rating)}
                        />
                      </div> */}
                      ...
                      {question.type === 'options' && (
                        <div className="options-container">
                          <QuestionOptions 
                            ...
                            onChange={(option) => handleOptionChange(question.id, option)}
                          />
                        </div>
                      )}
                      ...
                      <Form.Control
                        ...
                        onChange={(e) => handleDescriptionChange(question.id, e)}
    ...
    <div className="d-flex gap-2 justify-content-center">
      {[1, 2, 3, 4, 5].map(rating => (
        <div key={rating} className="text-center">
          ...
          />
          <div className="rating-label" style={{fontSize: '10px', marginTop: '4px', color: rating <= currentRating ? 'var(--star-color)' : '#aaa'}}>
            {rating}
          </div>
        </div>
      ))}
...
// Update the QuestionOptions component
const QuestionOptions = ({ question, value, onChange }) => {
  ...
  return (
    <div className="dashboard-layout">
          ...
          <div className="d-flex align-items-center order-lg-last">
            <Dropdown>
              ...
              <Dropdown.Menu align="end" className="modern-dropdown">
                <Dropdown.Item onClick={handleLogout} className="dropdown-item-modern text-danger">
      ...
      {/* Modals */}
      <Modal show={showPreview} onHide={() => setShowPreview(false)} centered className="modern-modal" size="lg">
      ...
      >
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowAnalysis(false)} className="btn-modern">
            Close
          </Button>
        </Modal.Footer>
      </Modal>
